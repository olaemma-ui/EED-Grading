...
public class Utils {
    ...
    /**
     * @param max this is the maximum number to be generated
     * @param min this is the minimum number to be generated
     * @param count this is the number / how many numbers should be generated
     * */
    public int[] genUniqueNum(int max, int min, int count){
        int k = 0;
        ...
        for (int i = 0; i < count; i++) {
            ...
            while (gen){
                gen = false;
                k = (new Random().nextInt(max+1 - min)+min);
                for (int j = 0; j < count; j++) {
                    if (k == numbs[j]) gen = true;
                }
            }
            numbs[i] = k;
        }
